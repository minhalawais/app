...
async def add_customer(data, user_role, current_user_id, ip_address, user_agent, company_id):
    try:
        ...
        # Name handling (Option A): parse single name or first/last names
        raw_name = (data.get('name') or f"{data.get('first_name', '')} {data.get('last_name', '')}").strip()
...
async def validate_customer_data(data, is_update=False, customer_id=None):
    ...
    # If single name field is provided, parse it into first_name and last_name
    ...
    # Note: last_name is optional under Option A to allow single-word names (e.g. "Minhal")
    required_fields = [
        'first_name', 'cnic', 'phone_1', 'email', 
