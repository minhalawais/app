...
def _apply_role_scope(query, company_id, user_role, employee_id=None):
    if user_role == 'super_admin':
        return query
    elif user_role == 'auditor':
        return query.filter(Invoice.company_id == company_id, Invoice.is_active == True)
    elif user_role in ['company_owner', 'manager', 'employee', 'technician', 'recovery_agent']:
        return query.filter(Invoice.company_id == company_id)
    return query.filter(Invoice.company_id == company_id)
...
def get_enhanced_invoice_by_id(id, company_id, user_role):
    try:
        ...
        # Get issuing company branding & profile
        company_data = None
        try:
            from app.crud.company_crud import get_company_profile
            company_data = get_company_profile(invoice.company_id)
        except Exception as comp_err:
            logger.error(f"Error attaching company info to invoice {id}: {str(comp_err)}")

        # Enhanced invoice data with ALL customer, company branding, and service plan details
        return {
            'id': str(invoice.id),
            'invoice_number': invoice.invoice_number,
            'company_id': str(invoice.company_id),
